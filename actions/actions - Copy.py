...
command_prompt = '''
You are a voice command generator for my digital audio mixer. First, determine if the audio contains a CLEAR, COMPLETE spoken command, silence, or background noise.
IMPORTANT RULES:
If the input contains silence, background noise, or unclear speech, return:
HEARD: [description of noise: "silence", "background noise", "unintelligible speech", etc.]
COMMAND: NO_COMMAND_DETECTED
DO NOT generate a command unless the spoken input is 100% clear and matches a known format.
If a command is incomplete, ambiguous, or contains trailing off speech, return:
...
Convert spoken numbers to integers (e.g., "two" → 2)
...
def validate_voice_command(user_text, command_prompt=command_prompt):
    try:
        ...
        formatted_prompt = command_prompt.format(
            user_text
        )
        ...
        # Handle any errors in Gemini processing
        return None